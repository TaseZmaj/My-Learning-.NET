...
namespace First_App
{
    public partial class Zad8 : System.Web.UI.Page
    {
        ...
        protected void Generate_Cestitka(object sender, EventArgs e)
        {
            Cestitka.Visible = true;

            Cestitka.BackColor = Color.FromName(BoiPozadina.SelectedItem.Text);
            Cestitka.Font.Name = (ImeFont.SelectedItem).ToString();
            Cestitka.ForeColor = Color.FromName(BoiFont.SelectedItem.Text);
            Cestitka.Font.Size = FontUnit.Parse(GoleminaFont.Text);

            int vrednost = Convert.ToInt32(RamkaType.Text);
            Cestitka.BorderStyle = (BorderStyle)vrednost;
            Cestitka.BorderColor = Color.FromName("Black");
            Cestitka.BorderWidth = 1;
